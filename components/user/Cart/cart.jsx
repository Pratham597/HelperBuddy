    ...
    try {
      ...
      const data = {
        ...
        services: cart.map((item) => ({
          serviceId: item.id,
          timeline: formatDate(item.timeline.date) + " " + item.timeline.timeSlot,
        })),
      }

      const res = await axios.post(`${process.env.NEXT_PUBLIC_URL}/api/user/checkout`, data, {
        headers: {
      ...
      <div className="container mx-auto px-4 py-8 animate-fadeIn">
                      ...
                      {cart.map((item, index) => (
                        <li key={index} className="pb-4 border-b last:border-none animate-fadeIn">
                          ...
                          <div className="flex justify-between items-center mb-4">
                            <div className="flex items-center space-x-4">
                              <img
                                src={item.image || "/placeholder.jpg" || "/placeholder.svg"}
                                alt={item.name}
                                className="w-20 h-20 rounded-md object-cover transition-transform duration-300 hover:scale-105"
                              />
                              <div>
        ...
        <div className="mt-8 text-center animate-slideInFromBottom">
          <Button
            ...
            disabled={!shippingAddress || cart.length === 0 || cart.some((item) => !item.timeline) || isLoading}
          >
            {isLoading ? (
              <>
